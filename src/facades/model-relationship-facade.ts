...
import {
  ...
  IObjectionModelRelationshipSchema,
...
export class ModelRelationshipFacade<T extends Model> {
  ...
  public getRelationships(opt?: IRelationshipDebugOptions<T>): IObjectionModelRelationship<T> {
    if (opt) {
      _.isFunction(opt?.log) ? opt.log(this._relationshipModel) : console.log(this._relationshipModel);
    }
  ...
  private getRelationshipName(modelClass: ModelClass<T> | string, relation: RelationshipEnum): string {
    if (modelClass instanceof String || typeof modelClass == 'string') return modelClass.toString();
    return relation == RelationshipEnum.HasManyRelation || relation == RelationshipEnum.ManyToManyRelation
      ? StringUtils.pluralize(StringUtils.toLowerFirstLetter(modelClass.name))
  ...
  private getFromField(modelClass: ModelClass<T> | string, relation: RelationshipEnum): string {
    if (relation != RelationshipEnum.BelongsToOneRelation) return 'id';
  ...
  private geToField(modelClass: ModelClass<T> | string, relation: RelationshipEnum): string {
    if (relation != RelationshipEnum.HasManyRelation && relation != RelationshipEnum.HasOneRelation) return 'id';
  ...
  private getTableName(modelClass: ModelClass<T> | string): string {
    return modelClass instanceof String || typeof modelClass == 'string' ? modelClass.toString() : modelClass.tableName;
  }
  ...
  public add(
    ...
    relationshipTypeEnum: RelationshipEnum,
    config?: IObjectionModelRelationshipAddConfig
  ): this {
    if (
      ((modelClass instanceof String || typeof modelClass == 'string') && !modelClass) ||
      !(modelClass as ModelClass<T>).tableName
    )
    ...
    const { fromTable, toTable, fromField, toField, relationName, ...joinConfig } = config || {};
    ...
    if (
      relationshipTypeEnum == RelationshipEnum.ManyToManyRelation ||
      relationshipTypeEnum == RelationshipEnum.HasOneThroughRelation
    ) {
